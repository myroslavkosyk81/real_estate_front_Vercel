...
export default function CreateListing() {
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto h-[calc(100vh_-_14vh)]'>
        ...
        {t('create_list.create')}</h1>
        <form onSubmit={handleSubmit} className='flex flex-col sm:flex-row gap-4'>
            <div className='flex flex-col gap-4 flex-1'>
                <input type="text" placeholder={t('create_list.name')} className='border p-3
                rounded-lg' id='name' maxLength='62' minLength='7' required
                onChange={handleChange}
                value={formData.name} />

                <textarea type="text" placeholder={t('create_list.description')} className='border p-3
                rounded-lg' id='description' required onChange={handleChange} value={formData.description} />

                <input type="text" placeholder={t('create_list.address')} className='border p-3
                ...
                <div className='flex gap-6 flex-wrap'>
                    <div className='flex gap-2'>
                        ...
                        <span>{t('create_list.sale')}</span>
                    </div>
                    <div className='flex gap-2'>
                        <input type="checkbox" id='rent' className='w-5'
                        onChange={handleChange} 
                        checked={formData.type === 'rent'} />
                        <span>{t('create_list.rent')}</span>
                    </div>
                    <div className='flex gap-2'>
                        <input type="checkbox" id='parking' className='w-5'
                        onChange={handleChange}
                        checked={formData.parking} />
                        <span>{t('create_list.parking')}</span>
                    </div>
                    <div className='flex gap-2'>
                        <input type="checkbox" id='furnished' className='w-5'
                        onChange={handleChange}
                        checked={formData.furnished}  />
                        <span>{t('create_list.furnished')}</span>
                    </div>
                    <div className='flex gap-2'>
                        <input type="checkbox" id='offer' className='w-5'
                        onChange={handleChange}
                        checked={formData.offer}  />
                        <span>{t('create_list.offer')}</span>
                    </div>
                </div>
                <div className='flex flex-wrap gap-6'>
                    ...
                    <div className='flex items-center gap-2'>
                        ...
                        <div  className='flex flex-col items-center'>
                           <p>{t('create_list.reg_price')}</p> 
                           <span className='text-xs'>({t('create_list.per_month')})</span>
                        </div>
            ...
            <div className='flex flex-col flex-1 gap-4'>
                <p className='font-semibold'>{t('create_list.images')}
                <span className='font-normal text-gray-600 ml-2'>{t('create_list.images_cover')}</span>
                </p>
                ...
                </p>
                {
                    formData.imageUrls.length > 0 && formData.imageUrls.map((url, index) => (
                        <div key={url} className='flex justify-between p-3 border items-center'>
                            ...
                            hover:opacity-75'>{t('create_list.delete_image')}</button>
                        </div>
                    ))
                }
                <button disabled={loading || uploading} className='p-3 bg-slate-700 text-white rounded-lg uppercase
                hover:opacity-95 disabled:opacity-80'>
                    {loading ? <span>{t('create_list.creating')}</span> : <span>{t('create_list.create_button')}</span>}
                </button>
